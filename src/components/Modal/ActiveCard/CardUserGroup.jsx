...
import { selectCurrentActiveBoard } from "~/redux/features/activeBoardSlice"
...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  // Lọc các member trong card bằng các members có trong board
  const board = useSelector(selectCurrentActiveBoard)
  // const FE_CardMembers = board.FE_allUsers.filter((user) =>
  //   cardMemberIds.includes(user._id),
  // )
  // Sắp xếp hiển theo theo thứ tự theo cardMemberIds
  const FE_CardMembers_2 = cardMemberIds.map((userId) => {
    return board.FE_allUsers.find((user) => user._id === userId)
  })
  // Xử lí update card members
