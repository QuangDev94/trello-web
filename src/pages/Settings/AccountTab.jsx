...
function AccountTab() {
  ...
  const submitChangeGeneralInformation = (data) => {
    ...
    // Gọi API...
    toast
      .promise(dispatch(updateUserApiThunk({ displayName })), {
        pending: "Updating ...",
      })
      .then((res) => {
        if (!res.error) {
  ...
  const uploadAvatar = (e) => {
    ...
    // Gọi API...
    toast
      .promise(dispatch(updateUserApiThunk(reqData)), {
        pending: "Updating ...",
      })
      .then((res) => {
        if (!res.error) {
        ...
        // Lưu ý: dù có lỗi hoặc thành công thì cũng phải clear giá trị của file input, nếu ko sẽ ko thể chọn cùng 1 file liên tiếp được
        e.target.value = ""
      })
  }
